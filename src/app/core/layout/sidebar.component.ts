...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, SafeHtmlPipe, MascotComponent],
  template: `
    @if (sidebar.open()) {
      <div class="fixed inset-0 z-30 bg-black/70 backdrop-blur-sm lg:hidden" (click)="sidebar.close()" role="presentation"></div>
    }

    <aside class="fixed top-0 left-0 z-40 h-dvh w-sidebar flex flex-col select-none transition-transform duration-300 -translate-x-full lg:translate-x-0" [class.translate-x-0]="sidebar.open()" style="background-color: var(--color-sidebar); border-right: 1px solid var(--color-border);">
      <div class="flex flex-col px-5 pt-6 pb-5 flex-shrink-0" style="border-bottom: 1px solid var(--color-border);">
        <div class="flex items-start justify-between mb-3">
          ...
          <button (click)="sidebar.close()" class="lg:hidden p-1.5 rounded-md transition-colors mt-0.5" style="color: var(--color-sidebar-muted);" aria-label="Close sidebar">
            <svg viewBox="0 0 24 24" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
          </button>
        </div>
      ...
      <nav class="flex-1 overflow-y-auto py-4 px-3" role="navigation" aria-label="Main navigation">
        <div class="space-y-0.5">
          @for (item of primaryItems(); track item.route) {
            <a [routerLink]="item.route" (click)="sidebar.close()" class="group flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all duration-150 relative"
              [class.nav-active]="isActive(item.route)" [class.nav-inactive]="!isActive(item.route)"
      ...
      <div class="px-4 py-4 flex-shrink-0" style="border-top: 1px solid var(--color-border);">
        <button (click)="logout()" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all duration-150" style="color: var(--color-sidebar-muted);">
          <svg viewBox="0 0 24 24" class="w-[18px] h-[18px] flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
          <span>Sign out</span>
        </button>
      </div>
    </aside>
    ...
    <style>
      .nav-active { background-color: rgba(61,184,255,0.1); color: var(--color-accent); }
      .nav-inactive { color: var(--color-sidebar-muted); }
      .nav-inactive:hover { color: var(--color-text); background-color: var(--color-sidebar-hover); }
    </style>
  `,
})
export class SidebarComponent {
  ...
  protected readonly primaryItems = computed(() => {
    ...
    if (role === 'admin') return this.allNavItems.filter(i => ['/admin', '/exercises', '/routines', '/sessions'].includes(i.route));
