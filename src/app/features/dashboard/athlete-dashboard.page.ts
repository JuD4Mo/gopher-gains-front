...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, DatePipe, MascotComponent, SafeHtmlPipe],
  template: `
    <div class="p-6 lg:p-10 max-w-[1200px] mx-auto">

      <!-- Hero zone -->
      <div class="relative rounded-2xl overflow-hidden mb-8" style="background-color: var(--color-card); border: 1px solid var(--color-border);">
        <div class="absolute top-0 left-0 right-0 h-0.5" style="background: linear-gradient(90deg, var(--color-accent) 0%, transparent 60%);"></div>

        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-5 p-6 lg:p-8">
          <div class="flex-shrink-0">
            <app-mascot variant="celebrating" size="xl" alt="Gopher ready to train" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-[11px] font-mono font-semibold uppercase tracking-[0.12em] mb-1" style="color: var(--color-accent);">My Dashboard</p>
            <h1 class="text-2xl lg:text-3xl font-bold font-display tracking-tight leading-tight mb-1" style="color: var(--color-text);">What should I do today?</h1>
          ...
          <div class="flex-shrink-0 flex flex-col gap-2 w-full sm:w-auto">
            <a routerLink="/sessions/new" class="btn-primary flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold rounded-xl">
              <span class="w-4 h-4 [&>svg]:w-full [&>svg]:h-full" [innerHTML]="icons.plus | safeHtml"></span>
              Start Session
            </a>
            <a routerLink="/sessions" class="btn-secondary flex items-center justify-center px-6 py-2.5 text-sm rounded-xl">View History</a>
          </div>
        </div>

        <!-- Weekly bar -->
        <div class="flex items-center gap-3 px-6 lg:px-8 py-4" style="border-top: 1px solid var(--color-border); background-color: rgba(0,0,0,0.12);">
          ...
          <div class="flex items-center gap-1.5 flex-1">
            @for (day of weekDays; track $index) {
              <div class="w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold font-mono transition-all"
                [style.background-color]="$index <= todayIndex ? 'rgba(61,184,255,0.15)' : 'rgba(255,255,255,0.03)'"
                [style.border]="$index === todayIndex ? '1px solid rgba(61,184,255,0.6)' : '1px solid var(--color-border)'"
        ...
        <div class="flex-1 min-w-0 flex flex-col gap-8">
          ...
          <div class="rounded-xl" style="background-color: var(--color-card); border: 1px solid var(--color-border);">
            ...
            @if (routines().length === 0) {
              <div class="flex flex-col items-center justify-center py-10 gap-3 px-6">
                <app-mascot variant="thinking" size="lg" alt="No routines yet" />
              ...
              <div class="divide-y" style="border-color: var(--color-border);">
                @for (r of routines(); track r.id) {
                  <a [routerLink]="'/routines/' + r.id" class="group flex items-center gap-4 px-6 py-4 transition-all duration-150 hover:bg-[var(--color-card-hover)]">
                    <div class="w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center transition-transform group-hover:scale-105" style="background-color: rgba(183,148,244,0.12); color: #B794F4;">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      @if (r.description) { <p class="text-xs font-mono mt-0.5 truncate" style="color: var(--color-muted);">{{ r.description }}</p> }
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                      <a routerLink="/sessions/new" class="text-[11px] font-semibold px-3 py-1.5 rounded-lg transition-all" style="background-color: rgba(61,184,255,0.12); color: var(--color-accent); border: 1px solid rgba(61,184,255,0.2);" (click)="$event.stopPropagation()">Start</a>
          ...
          <div class="rounded-xl" style="background-color: var(--color-card); border: 1px solid var(--color-border);">
            ...
            @if (recentSessions().length === 0) {
              <div class="flex flex-col items-center justify-center py-10 gap-3 px-6">
                <app-mascot variant="clipboard" size="lg" alt="No sessions logged yet" />
                <div class="text-center">
              ...
              <div class="divide-y" style="border-color: var(--color-border);">
                @for (s of recentSessions(); track s.id) {
                  <a [routerLink]="'/sessions/' + s.id" class="group flex items-center gap-4 px-6 py-4 transition-all hover:bg-[var(--color-card-hover)]">
                    <div class="w-9 h-9 rounded-xl flex-shrink-0 flex items-center justify-center" style="background-color: rgba(246,201,14,0.1); color: #F6C90E;">
                      <div class="w-4 h-4 [&>svg]:w-full [&>svg]:h-full" [innerHTML]="icons.sessions | safeHtml"></div>
                    </div>
                    <div class="flex-1 min-w-0">
                      <p class="text-sm font-medium leading-snug truncate" style="color: var(--color-text);">{{ s.observations || 'Workout session' }}</p>
